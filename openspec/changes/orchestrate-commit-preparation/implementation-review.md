...
**Summary:** Задача 3.16 не полностью подтверждена: тест подготовки объединяет две истории (F2), helper-бинарники собираются повторно (F5), а безопасный отказ не доказывает неизменность Git (F3).
